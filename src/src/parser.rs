...
impl Parser {
    ...
    #[throws]
    pub fn identifier_list(&mut self) -> Node {
        let idents = Vec::new();

        idents.push(self.next_ensure()?);
    ...
    pub fn next_ensure(&mut self, token: Token) -> Result<&TokenWrapper, String> {
        let next = self.next()?;
        if next.token != token {
            return Self::error(next, &format!("Exptected {token:?} found {:?}", next.token));
        }
        Ok(next)
    }
